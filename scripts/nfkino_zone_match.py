"""Zone and adjacency matching over NFkino seatmap data.

Input seat shape (records from nfkino_checkout.py's
seats_from_seats_selector()): {"row": int, "column": int,
"state": "available"|"booked", "type": str, ...other fields ignored}.
Seats whose type differs from the room's majority type (confirmed:
wheelchair seats are flagged `type: "wheelchair"`, everything else `""`)
are excluded - they're a different ticket category than the standard
seats being searched for.

Logic (zone bucketing, contiguous-run search) is otherwise identical to
filmgrail_zone_match.py/odeon_zone_match.py/ebillett_zone_match.py - kept
as its own module rather than shared so each chain's matcher can diverge
independently if any one vendor's seat shape changes later.
"""
import argparse
import json
import sys
...
def standard_seats(seats):
    ...
    `type` value - excludes the minority `wheelchair` seats, the same
    majority-type reasoning used in filmgrail_zone_match.py."""
    if not seats:
        return []
    type_counts = {}
    for s in seats:
...
def main():
    parser = argparse.ArgumentParser(
        description='Find N contiguous available seats in a zone from extracted NFkino seatmap JSON (read from stdin)')
