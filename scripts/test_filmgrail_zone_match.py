import unittest
from filmgrail_zone_match import zone_bounds, row_zone, col_zone, find_contiguous_run, find_match, standard_seats
...
class TestFindContiguousRun(unittest.TestCase):
    ...
    def test_returns_none_when_not_enough_available(self):
        row = [seat(0, 0), seat(0, 1, state='booked')]
        self.assertIsNone(find_contiguous_run(row, 2))
    ...
    def test_column_gap_breaks_the_run_even_if_available(self):
        # column 2-7 missing entirely (e.g. an aisle) - 1 and 8 aren't adjacent
