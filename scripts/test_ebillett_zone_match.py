import unittest
from ebillett_zone_match import zone_bounds, row_zone, col_zone, find_contiguous_run, find_match, standard_seats


def seat(row, column, state='available'):
    return {'row': row, 'column': column, 'state': state, 'type': '', 'rowSymbol': str(row)}


class TestStandardSeats(unittest.TestCase):
    def test_returns_all_seats_unchanged(self):
        seats = [seat(0, 0), seat(0, 1, state='booked')]
        self.assertEqual(standard_seats(seats), seats)

    def test_empty_input(self):
        self.assertEqual(standard_seats([]), [])
